...
2. 何が問題なのかの簡潔な説明
3. その根拠がどこにあるか
4. one-off か recurring かの暫定判断
5. いま実装すべきとは限らないことの明示
...
1. 問題が1〜3文で説明できる
2. evidence source が示されている
3. one-off / repeated / regression / structural gap の暫定分類がある
4. affected layer が分かる
5. 現在の user instruction と矛盾していない
6. 自動実行前提になっていない
7. 何がまだ unknown かが示されている
8. approval が必要かどうかが明記されている
9. backlog item は authorization ではないと明示されている
