...
Improvement Candidate schema v1 の source of truth はこの section とする。

原則:

- candidate = inventory, not authorization
- schema は review と handoff のための記録形式であり、task creation / approval / execution permission を表さない
...
- `approval_required` は「承認が必要か」を示す項目であり、「承認済み」を示さない
- `next_review_step` は次の review action の候補であり、implementation instruction ではない
- `source` と `evidence` は Quality finding / Weekly Report / review note などの観測根拠を残すために分ける
- `auto_apply_allowed` は v1 では常に `false` とし、candidate から自動実行許可を導出しない

任意項目:

- `severity` を入れる場合は問題の重さを示す
- `risk_level` は変更の危険度を示す
- `proposed_change_type` を入れる場合は docs clarification / reporting refinement / verification gap reduction など non-authorizing な弱い分類に留める
- `allowed_actions` / `forbidden_actions` は schema 本体には入れず、boundary rule 側で扱う

この schema は最小構成を固定するためのものであり、status や approval class を増やす前提にはしない。
...
- `auto_apply_allowed = false` は v1 の固定前提であり、auto-approve / auto-dispatch / auto-mutate を許可しない
- candidate preview や backlog item が存在しても task creation / implementation permission は発生しない
